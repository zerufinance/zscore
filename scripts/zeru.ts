 ...
 *   DISCOVERY_API — Optional, Discovery API base URL (default: https://agentapi.zpass.ai)
 */
...
const DEFAULT_DISCOVERY_API = "https://agentapi.zpass.ai";

/**
 * Notify the backend immediately after a successful mint.
 * Ensures the agent appears in the Discovery API within ~1–2s instead of 15–45s.
 * Non-fatal: on failure we only log a warning; the background indexer will pick it up.
 */
async function notifyMint(
  txHash: string,
  chainId: number,
  registryAddress?: string
): Promise<{ agents?: unknown[] } | null> {
  const base = process.env.DISCOVERY_API ?? DEFAULT_DISCOVERY_API;
  const response = await fetch(`${base}/v1/discovery/notify-mint`, {
    method: "POST",
  ...
  if (!response.ok) {
    ...
    console.warn("  notify-mint failed (non-fatal):", err);
    return null;
  }
...
async function cmdRegister(flags: Record<string, string>) {
  ...
  console.log("Step 3/5: Updating document with real agentId...");
  ...
  console.log("Step 4/5: Notifying backend (instant indexing)...");
  const notifyResult = await notifyMint(txHash, config.chainId, config.identityRegistryAddress);
  if (notifyResult?.agents?.length) {
    console.log(`  Indexed: ${notifyResult.agents.length} agent(s)`);
  }
