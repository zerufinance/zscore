...
const CHAIN_DEFAULTS = {
    8453: {
        ...
        zscoreApiBase: "https://agentapi.zpass.ai",
    },
    84532: {
...
/** Resolve a ZscoreConfig by filling in defaults for known chains. */
export function resolveConfig(config) {
    ...
    const zscoreApiBase = config.zscoreApiBase ?? defaults?.zscoreApiBase;
    ...
    if (!zscoreApiBase) {
        throw new Error(`zscoreApiBase required for chainId ${config.chainId}`);
    }
    return {
        ...
        zscoreApiBase: stripTrailingSlash(zscoreApiBase),
        discoveryApiKey: config.discoveryApiKey,
        source: config.source ?? "zscore",
    };
}
...
export function discoveryUrl(rc) {
    return `${rc.zscoreApiBase}/v1/discovery`;
}
